...
export function HeroStatsBar() {
  ...
  return (
    <div
      ref={rootRef}
      className="border-t border-[rgba(46,109,164,0.4)]"
      style={{ background: "rgba(26,60,94,0.22)" }}
    >
      <div
      ...
      >
        {stats.map((stat, i) => (
          <div
            key={i}
            className="bg-[rgba(8,12,18,0.86)] p-4 backdrop-blur-sm sm:p-5 md:p-6 lg:p-7 group transition-all duration-300 hover:bg-[rgba(26,60,94,0.26)]"
          >
            <div className="mb-4 h-px w-8 bg-[#2E6DA4]" />
            <span
              data-value={stat.value}
              className="animate-number text-[clamp(1.5rem,3.5vw,2.4rem)] font-semibold text-[#9EDBFF] leading-none mb-1.5 block group-hover:scale-110 transition-transform duration-300"
            >
              {stat.value}
            </span>
            <span
              className="text-[0.55rem] sm:text-[0.6rem] tracking-[0.1em] sm:tracking-[0.12em] uppercase text-[#DDEBFA] leading-[1.4]"
